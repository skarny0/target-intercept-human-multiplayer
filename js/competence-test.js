...
const firebaseConfig_db1 = {
    apiKey: "AIzaSyDHFmVno0gBcFlJiTWCSj6-bwv70RNU47I",
    authDomain: "multiplayer-competence.firebaseapp.com",
    databaseURL: "https://multiplayer-competence-default-rtdb.firebaseio.com",
    projectId: "multiplayer-competence",
...
if (DEBUG){
   studyId    = "multiplayer-test-0619-debug";
} else {
    studyId   = "multiplayer-test-0619";
}
...
function writeGameDatabase(){
    ...
    // let pathUID1 = studyId + '/participantData/' + firebaseUserId1 + '/keyUID';
    // writeRealtimeDatabase(db2, pathUID1, firebaseUserId1);
    // writeRealtimeDatabase(db1, pathUID1, firebaseUserId1);
    ...
    // let path14  = studyId + '/participantData/' + firebaseUserId1 + '/competence' + '/AIClicks_Adjusted';
    // let path8   = studyId + '/participantData/' + firebaseUserId1 + '/competence' + '/AIcaughtTargets';
    // let path9   = studyId + '/participantData/' + firebaseUserId1 + '/competence' + '/AIClicks';
...
async function endGame() {
    ...
    if (currentRound == 1) {//&& numSurveyCompleted < 3) {
        await runGameSequence("Awesome performance! You were able to acquire: " + player.score + " points.")
    ...
    objects.forEach((obj, index) => {
        if (obj.active) {
            ...
            if (!obj.intercepted && checkCollision(AIplayer, obj)) { // MS5: added a condition
                // Collision detected
                obj.intercepted   = false; // Added this flage to make sure the object despawns after being caught  
...
function drawScore() {
    ...
    scoreCtx.fillText('Score: ' + score, 10, 40); // Adjust the positioning as needed
