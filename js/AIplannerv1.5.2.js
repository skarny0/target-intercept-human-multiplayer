...
// MS32
function runAIPlanner( objects, player , observableRadius , center, whAgent, stabilityThreshold, prevBestSol, frame, alpha, isBottomFeeder) {
    ...
    if ((isBottomFeeder) && (whAgent == 'collab')) {
        for (let idx in objectValues) {            
            // flip values so that high values are not low and vise versa
            objectValues[idx] = -objectValues[idx];  
        }
    } 
    
    // Run the planner on these normalized coordinates
    // MS8
    [ bestSol ] = planSingleFrame(playerStartX, playerStartY, playerSpeed, objectPositionsX, objectPositionsY, 
        objectVelocitiesX, objectVelocitiesY, objectValues, circleRadius, ID, alpha, frame);
    ...
    if ((stabilityThreshold != null) && (prevBestSol != null)) {
        ...
        if (currentBestID != lastBestID) {
            ...
            let valuePreviousPlan = prevBestSol.maxValue; 
            let ratioScores = ( valueCurrentPlan + 0.0001 ) / ( valuePreviousPlan + 0.0001 );

            // If there is insufficient improvement, consider reverting back to the plan of intercepting the previous ID
            if ((ratioScores < stabilityThreshold) && (lastBestID != -1)) {
                
                // Does it still exist in the current list?
                let objectIdx = findIndex(lastBestID, ID);
                if (objectIdx != -1) {
                    // Is the previous target still interceptable?
                    let [success, timeToIntercept, interceptPosX, interceptPosY, distance] = attemptIntercept(
                        playerStartX, playerStartY, playerSpeed,
                        objectPositionsX[objectIdx], objectPositionsY[objectIdx],
                    ...
                    if (success) {
                        ...
                        bestSol.ID = prevBestSol.ID;
                        bestSol.maxValue = prevBestSol.maxValue;
                        bestSol.minDistance = distance;
                        bestSol.minTime = timeToIntercept;
                        bestSol.interceptionOrder = prevBestSol.interceptionOrder.slice();
                        bestSol.interceptTimes = prevBestSol.interceptTimes.slice();
                        
                        let soll = bestSol.interceptionOrder.length;
                        bestSol.interceptLocations = [];
                        bestSol.interceptTimes = [];
                        for (let k=0; k<soll; k++) {
                            bestSol.interceptLocations.push( [ prevBestSol.interceptLocations[k][0] , prevBestSol.interceptLocations[k][1] ] );
                            
                        }            
                        bestSol.interceptLocations[0][0] = ( interceptPosX * observableRadius ) + center.x;
                        bestSol.interceptLocations[0][1] = ( interceptPosY * observableRadius ) + center.y;                   
                        bestSol.interceptTimes[0] = timeToIntercept;
                    } 
...
function planSingleFrame(playerStartX, playerStartY, playerSpeed, objectPositionsX, objectPositionsY, objectVelocitiesX, objectVelocitiesY, objectValues, circleRadius, ID, alpha, frame ) {
    ...
    // debugging
    //if ((numObjects > 1) && ( frame==159)) {
    //    console.log( "here");
    //}
    
    // Initialize the best initial solution as going to the center
    let bestSol = {
        maxValue: -Infinity,
    ...
    let sortedTimes = sortArray( timeToLeaveCircle );
                ...
                if (planningDepth >= 1) {
                    ...
                    for (let j = 0; j < planningDepth; j++) {
                        ...
                        if (success) {
                            ...
                            // MS30
                            for (kk=0; kk<numObjects; kk++) {
                                currentObjectPositionsX[kk] = currentObjectPositionsX[kk] + objectVelocitiesX[kk] * timeToIntercept;
                                currentObjectPositionsY[kk] = currentObjectPositionsY[kk] + objectVelocitiesY[kk] * timeToIntercept;
                            }
                        }
